...
class NNAgent:
    def __init__(self, training_data, testing_data, target_values, features, epochs: int = 100, training_size: int = 1000, learning_rate: float = 0.1):
        ...
        # create main model
        self.model = OurModel(input_shape=(12*len(features),), output_shape=1)
    ...
    def generate_features(self, dataset, training_size: int = None, testing: bool = False):
        ...
        if not testing:
            ...
            feature_vector = np.empty((training_size,12*len(self.features)))
            ...
            feature_vector = np.empty((self.testing_data['unit_number'].max(),12*len(self.features)))
            target_vector = None
        ...
        for idx,engine in enumerate(engine_numbers):
            ...
            feature_vector[idx, 4*len(self.features):5*len(self.features)] = dataset.loc[ 
                (dataset['unit_number']==engine) & (dataset['time'] == max(T_end - 5, T_start)), self.features].to_numpy()
            
            feature_vector[idx, 5*len(self.features):6*len(self.features)] = dataset.loc[ 
                (dataset['unit_number']==engine) & (dataset['time'] == max(T_end - 10, T_start)), self.features].to_numpy()

            feature_vector[idx, 6*len(self.features):7*len(self.features)] = dataset.loc[ 
                (dataset['unit_number']==engine) & (dataset['time'] == max(T_end - 15, T_start)), self.features].to_numpy()
            
            feature_vector[idx, 7*len(self.features):8*len(self.features)] = dataset.loc[ 
                (dataset['unit_number']==engine) & (dataset['time'] == max(T_end - 20, T_start)), self.features].to_numpy()
            
            feature_vector[idx, 8*len(self.features):9*len(self.features)] = dataset.loc[ 
                (dataset['unit_number']==engine) & (dataset['time'] == max(T_end - 25, T_start)), self.features].to_numpy()

            feature_vector[idx, 9*len(self.features):10*len(self.features)] = dataset.loc[ 
                (dataset['unit_number']==engine) & (dataset['time'] == max(T_end - 2, T_start)), self.features].to_numpy()

            feature_vector[idx, 10*len(self.features):11*len(self.features)] = dataset.loc[ 
                (dataset['unit_number']==engine) & (dataset['time'] == max(T_end - 7, T_start)), self.features].to_numpy()
            
            feature_vector[idx, 11*len(self.features):12*len(self.features)] = dataset.loc[ 
                (dataset['unit_number']==engine) & (dataset['time'] == max(T_end - 12, T_start)), self.features].to_numpy()
...
def main() -> None:
    ...
    s.set(xlabel='Size of Training Data', ylabel='No. of Episodes')
