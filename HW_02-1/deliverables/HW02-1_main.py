...
def main() -> int:
    ...
    if dataset == 'Imperial':
        order = [0,1,2,3]
        plt.plot(time, (acceleration_centrald+acceleration_euler_fw+acceleration_2nd_order)/3, 
                 color=[0.5,0.5,0.5], label='Average')
    ...
    # Plot Acceleration Error Difference to Measurement (Chichi)
    if dataset == 'Chichi':
        plt.figure()
        order = [0,1,2]
        plt.plot(time, acceleration_centrald - acceleration,'r-',
                label='From Central Diff. Velocity Profile (central diff.)')
        plt.plot(time, acceleration_euler_fw - acceleration,'b-', markersize=3,
                label='From Forward Diff. Velocity Profile (backward diff.)')
        plt.plot(time, acceleration_2nd_order - acceleration,'y--', markersize=3,
                label='From Displacement Profile (2nd order central diff.)')
        handles, labels = plt.gca().get_legend_handles_labels()
        plt.legend([handles[idx] for idx in order],[labels[idx] for idx in order])
        plt.title(str(dataset)+" Earthquake Acceleration Error")
        plt.xlabel("Time [s]")
        plt.ylabel("Acceleration (m/s2)")
        plt.show()

    # Plot Acceleration Error Difference to 1st/2nd Order (Imperial)
    if dataset == 'Imperial':
        plt.figure()
        order = [0,1]
